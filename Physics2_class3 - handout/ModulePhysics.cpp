...
bool PhysBody::Contains(int x, int y) const
{
	...
	for (b2Fixture* fixt = this->body->GetFixtureList(); fixt && !ret; fixt = fixt->GetNext())
	{
		if (fixt->GetShape()->TestPoint(trans, point))
		{
			ret = true;
		}
	}
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	trans.SetIdentity();
	b2RayCastInput ray;
	ray.p1.Set(PIXEL_TO_METERS(x) , 0.0f, 0.0f);
	ray.p2.(SetPIXEL_TO_METERS(x), 0.0f, 0.0f);
